...
def make_level_fn(days):
    """Pick a palette slot per day, scaled to THIS profile's own activity.

    Absolute thresholds (count > 50 == brightest) only work for someone with
    thousands of contributions a year; on a quieter profile every cell collapses
    into the two darkest greens. GitHub's own `data-level` is already quartiled
    per user, so prefer it and reserve the neon slot 5 for personal-best days.
    """
    counts = [d["count"] for d in days]
    best = max(counts) if counts else 0
    neon_floor = max(2, best * 0.8)
    ...
    def level_for(day):
        ...
        if gh:
            return min(int(gh), 4)
        # Fallback if data-level is missing: quarters of the personal best.
        return max(1, min(4, int(count / best * 4 + 0.5))) if best else 1

    return level_for


def build_grid(days):
    level_for = make_level_fn(days)
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">ce017@github: ~/contributions --graph</text>')
...
if __name__ == "__main__":
    with open(IN_PATH, encoding="utf-8") as fh:
        data = json.load(fh)
    svg = render(data)
    # Explicit UTF-8: open() defaults to the locale codepage on Windows.
