...
USERNAME = os.environ.get("GH_PROFILE_USER", "ce017")
...
def fetch_days():
    ...
    # Index the tooltips once by the cell id they point at. Searching the whole
    # tree per cell is O(n^2) over ~365 cells.
    tooltips = {
        t["for"]: t.get_text(strip=True)
    ...
    for td in cells:
        ...
        # GitHub's own 0-4 intensity, already scaled to THIS user's activity --
        # far better than absolute thresholds (see level_for in render_heatmap_svg).
