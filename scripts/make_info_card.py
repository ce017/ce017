...
W = int(os.environ.get("CARD_W", 480))
PAD = 20
TITLEBAR_H = 30
KEY_X = PAD
VAL_X = PAD + 92
...
# ("host",)                    -> "<handle>@github" + rule
# ("kv", key, value)           -> orange key + light value
# ("sec", title)               -> blue "— title —" rule
# ("bul", text)                -> green dot + light text
# ("gap",)                     -> vertical space
#
# Labels are free text -- they are not a fixed schema. Swap "Portfolio"/"Discord"
# for whatever you actually want listed.
ROWS = [
    ("host",),
    ("kv", "Now", "Roblox developer — games & systems"),
    ("kv", "Also", "Websites, web apps, tooling"),
    ("kv", "Edu", "Finance high school (current)"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "Roblox", "Luau, Studio, Rojo"),
    ("kv", "Web", "HTML, CSS, JS, Vercel"),
    ("kv", "Python", "Pillow, Satori renders"),
    ("kv", "Tools", "Git, GitHub Actions"),
    ("gap",),
    ("sec", "Links"),
    ("kv", "Portfolio", "ce-17.vercel.app"),
    ("kv", "Instagram", "@btcmoneyy"),
    ("kv", "Discord", "@.ceo2000_08"),
    ("kv", "Telegram", "@lce17"),
    ("kv", "Roblox", "ceo2000_08yt"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "100k+ visits across Roblox games"),
    ("bul", "Shipped sites for real companies"),
]

# Height follows the content, so editing ROWS never leaves dead space at the
# bottom (or clips the last line). CARD_H overrides if you need a fixed panel.
_units = sum(0.5 if row[0] == "gap" else 1 for row in ROWS)
H = int(os.environ.get("CARD_H", round(FIRST_BASELINE + _units * LINE_H + BOTTOM_PAD)))
...
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        continue
    if kind == "host":
        # Rule starts just past "<handle>@github", whose width depends on the
        # handle -- monospace at font-size 14 advances ~0.6em per glyph.
        rule_x = KEY_X + (len(HANDLE) + 7) * 14 * 0.6 + 10
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">{esc(HANDLE)}</tspan><tspan fill="{MUTED}">@</tspan>'
                 f'<tspan fill="{ACCENT}">github</tspan></text>'
                 f'<line x1="{rule_x:.1f}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
...
parts.append("</svg>")
svg = "".join(parts)
# Explicit UTF-8: open() defaults to the locale codepage (cp1252 on most Windows
# installs), which silently writes non-ASCII as bytes no SVG parser will accept.
